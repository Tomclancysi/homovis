...
! function() {
  ...
  t.prototype = {
    ...
    gradient: function(t) {
      var i = document.createElement("canvas"),
        a = i.getContext("2d"),
        s = a.createLinearGradient(0, 0, 0, 256);// 设置一条渐变线
      i.width = 1, i.height = 256;
      for (var e in t) s.addColorStop(e, t[e]); //设置 渐变直线上人的点的颜色
    ...
    changeGrayData: function (imgData){
      // 首先将数组转成uint8,4通道的那种，然后进行一个colorlize
  ...
  */
  L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
    changeGrayData(imgData){
      this._heat.changeGrayData(imgData)
    },
    colorize(d){
      this._heat._colorize(d, this._heat._grad)
    },
    ...
    setOptions: function(t) {// 在这设置一下新的color grad
